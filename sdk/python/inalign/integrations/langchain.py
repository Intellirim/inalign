"""
LangChain integration for InALign.

Provides callback handlers that automatically proxy all LangChain
tool calls and LLM requests through InALign for governance.
"""
...
class InALignCallback(BaseCallbackHandler if LANGCHAIN_AVAILABLE else object):
    ...
    Usage::
        ...
        from inalign.integrations import InALignCallback

        callback = InALignCallback(
            api_key="ask_xxx",
    ...
    def __init__(
        ...
        """Initialize the InALign callback.
        ...
        if not response.allowed:
            if self.block_on_violation:
                raise PermissionError(
                    f"InALign blocked tool '{action.tool}': {response.reason}"
                )
            # If not blocking, log and continue
            print(f"[InALign] Warning: Tool '{action.tool}' would be blocked: {response.reason}")
        ...
        **kwargs: Any,
        ...
        if self._current_action_id is None:
            ...
            if not response.allowed and self.block_on_violation:
                raise PermissionError(
                    f"InALign blocked tool '{tool_name}': {response.reason}"
                )
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        if not response.allowed and self.block_on_violation:
            raise PermissionError(
                f"InALign blocked LLM call to '{model_name}': {response.reason}"
            )
...
def create_governed_agent(
    ...
    **kwargs: Any,
    ...
    """Create an InALign callback for LangChain agents.
    ...
    Args:
        ...
        **kwargs: Additional arguments passed to InALignCallback.

    Returns:
        InALignCallback instance ready to use with LangChain.

    Example::

        from langchain.agents import AgentExecutor
        from inalign.integrations import create_governed_agent
