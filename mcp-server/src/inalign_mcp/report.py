...
SCRIPT_PLACEHOLDER
    ...
    script_tag = (
        ...
        "  html += '<div style=\"color:var(--muted);\">ID</div><div><code style=\"color:var(--accent);font-size:0.72rem;\">' + node.id + '</code></div>';\n"
        "  html += '<div style=\"color:var(--muted);\">Class</div><div style=\"color:'+color+';font-weight:600;\">' + node.class + '</div>';\n"
        "  if (node.ts) html += '<div style=\"color:var(--muted);\">Timestamp</div><div>' + node.ts + '</div>';\n"
        "  if (node.label) html += '<div style=\"color:var(--muted);\">Label</div><div>' + node.label + '</div>';\n"
        "  html += '</div>';\n"
        "\n"
        "  // Attributes\n"
        ...
        "        html += '<div style=\"padding:0.15rem 0;border-bottom:1px solid var(--border);cursor:pointer;\" onclick=\"window._ontoSelectById(\\'' + nb.node.id + '\\')\">';\n"
        "        html += arrow + ' <code style=\"color:var(--dim);\">' + nb.rel + '</code> ';\n"
        "        html += '<span style=\"color:'+nc+';\">' + nb.node.class + '</span> ';\n"
        "        html += (nb.node.label||nb.node.id) + '</div>';\n"
        "      });\n"
