  ...
  <div class="section" id="onto-graph-section" style="display:{'block' if onto_nodes > 0 else 'none'};">
    ...
    <div id="onto-detail" style="display:none;margin-top:0.8rem;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:1rem 1.2rem;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.6rem;">
        <h3 id="onto-detail-title" style="font-size:0.9rem;color:var(--accent);margin:0;">Node Detail</h3>
        <span onclick="document.getElementById('onto-detail').style.display='none';window._ontoSelectedNode=null;window._ontoDraw&&window._ontoDraw();" style="cursor:pointer;color:var(--muted);font-size:1.1rem;">&times;</span>
      </div>
      <div id="onto-detail-body" style="font-size:0.78rem;color:var(--text);"></div>
    </div>
  </div>
</div>
...
SCRIPT_PLACEHOLDER
    ...
    script_tag = (
        ...
        "  } catch(e) {\n"
        "    status.innerHTML = '<span style=\"color:var(--red)\">Error: ' + (e.message||'').replace(/</g,'&lt;') + '</span>';\n"
        ...
        "let _ontoInitialized = false;\n"
        "window._ontoSelectedNode = null;\n"
        ...
        "  const panel = document.getElementById('onto-detail');\n"
        "  const title = document.getElementById('onto-detail-title');\n"
        ...
        "  title.innerHTML = '<span style=\"color:'+color+';\">' + escHtml(node.class) + '</span> &mdash; ' + escHtml(node.label||node.id);\n"
        "\n"
        "  let html = '<div style=\"display:grid;grid-template-columns:100px 1fr;gap:0.3rem 0.8rem;margin-bottom:0.6rem;\">';\n"
        "  html += '<div style=\"color:var(--muted);\">ID</div><div><code style=\"color:var(--accent);font-size:0.72rem;\">' + escHtml(node.id) + '</code></div>';\n"
        "  html += '<div style=\"color:var(--muted);\">Class</div><div style=\"color:'+color+';font-weight:600;\">' + escHtml(node.class) + '</div>';\n"
        "  if (node.ts) html += '<div style=\"color:var(--muted);\">Timestamp</div><div>' + escHtml(node.ts) + '</div>';\n"
        "  if (node.label) html += '<div style=\"color:var(--muted);\">Label</div><div>' + escHtml(node.label) + '</div>';\n"
        "  html += '</div>';\n"
        "\n"
        "  // Attributes\n"
        ...
        "    html += '<div style=\"margin-top:0.5rem;\"><div style=\"color:var(--muted);font-size:0.72rem;margin-bottom:0.3rem;font-weight:600;\">Attributes</div>';\n"
        "    html += '<div style=\"background:var(--card);border-radius:4px;padding:0.5rem;font-size:0.72rem;max-height:150px;overflow-y:auto;\">';\n"
        "    attrKeys.forEach(k => {\n"
        "      let v = attrs[k];\n"
        "      if (typeof v === 'object') v = JSON.stringify(v);\n"
        "      if (typeof v === 'string' && v.length > 120) v = v.slice(0,120) + '...';\n"
        "      v = escHtml(v); k = escHtml(k);\n"
        "      // Risk-specific highlights\n"
        "      if (node.class === 'Risk' && (k === 'mitre_tactic' || k === 'mitre_techniques')) {\n"
        "        html += '<div><span style=\"color:var(--red);\">' + k + ':</span> <span style=\"color:var(--orange);\">' + v + '</span></div>';\n"
        "      } else {\n"
        "        html += '<div><span style=\"color:var(--dim);\">' + k + ':</span> ' + v + '</div>';\n"
        "      }\n"
        "    });\n"
        "    html += '</div></div>';\n"
        "  }\n"
        "\n"
        "  // Neighbors\n"
        "  if (window._ontoEdges && window._ontoNodeMap) {\n"
        "    const neighbors = [];\n"
        "    window._ontoEdges.forEach(e => {\n"
        "      if (e.s === node.id && window._ontoNodeMap[e.t]) neighbors.push({dir:'out', rel:e.r, node:window._ontoNodeMap[e.t]});\n"
        "      if (e.t === node.id && window._ontoNodeMap[e.s]) neighbors.push({dir:'in', rel:e.r, node:window._ontoNodeMap[e.s]});\n"
        "    });\n"
        "    if (neighbors.length > 0) {\n"
        "      html += '<div style=\"margin-top:0.5rem;\"><div style=\"color:var(--muted);font-size:0.72rem;margin-bottom:0.3rem;font-weight:600;\">Connected Nodes (' + neighbors.length + ')</div>';\n"
        "      html += '<div style=\"max-height:120px;overflow-y:auto;font-size:0.72rem;\">';\n"
        "      neighbors.slice(0,20).forEach(nb => {\n"
        "        const nc = COLORS[nb.node.class]||'#888';\n"
        "        const arrow = nb.dir==='out' ? '&rarr;' : '&larr;';\n"
        "        html += '<div style=\"padding:0.15rem 0;border-bottom:1px solid var(--border);cursor:pointer;\" onclick=\"window._ontoSelectById(\\'' + escHtml(nb.node.id) + '\\')\">';\n"
        "        html += arrow + ' <code style=\"color:var(--dim);\">' + escHtml(nb.rel) + '</code> ';\n"
        "        html += '<span style=\"color:'+nc+';\">' + escHtml(nb.node.class) + '</span> ';\n"
        "        html += escHtml(nb.node.label||nb.node.id) + '</div>';\n"
        "      });\n"
        "      if (neighbors.length > 20) html += '<div style=\"color:var(--dim);padding:0.2rem 0;\">... +' + (neighbors.length-20) + ' more</div>';\n"
        "      html += '</div></div>';\n"
        "    }\n"
        "  }\n"
        "  body.innerHTML = html;\n"
        "}\n"
        "\n"
        "// Highlight CQ-related nodes on canvas\n"
        "window._ontoHighlightCQ = function(cqKey) {\n"
        "  const cqData = _ONTO[cqKey] || {};\n"
        "  // Build set of relevant node IDs from CQ results\n"
        "  const ids = new Set();\n"
        "  if (cqKey === 'cq2') {\n"
        "    (cqData.patterns || []).forEach(p => {\n"
        "      // Highlight Risk nodes matching exfiltration\n"
        "      if (window._ontoNodes) window._ontoNodes.forEach(n => {\n"
        "        if (n.class === 'Risk' || n.class === 'Entity') ids.add(n.id);\n"
        "      });\n"
        "    });\n"
        "    // If no patterns, still highlight all Risk + Entity nodes\n"
        "    if (ids.size === 0 && window._ontoNodes) {\n"
        "      window._ontoNodes.forEach(n => { if (n.class === 'Risk' || n.class === 'Entity') ids.add(n.id); });\n"
        "    }\n"
        "  } else if (cqKey === 'cq3') {\n"
        "    (cqData.violations || []).forEach(v => {\n"
        "      if (window._ontoNodes) window._ontoNodes.forEach(n => {\n"
        "        if (n.class === 'Risk' || n.class === 'Policy') ids.add(n.id);\n"
        "      });\n"
        "    });\n"
        "    if (ids.size === 0 && window._ontoNodes) {\n"
        "      window._ontoNodes.forEach(n => { if (n.class === 'Risk' || n.class === 'Policy') ids.add(n.id); });\n"
        "    }\n"
        "  }\n"
        "  window._ontoHighlightSet = ids.size > 0 ? ids : null;\n"
        "  window._ontoSelectedNode = null;\n"
        "  if (window._ontoDraw) window._ontoDraw();\n"
        "  // Highlight the insight card border\n"
        "  document.querySelectorAll('.onto-insight-card').forEach(c => c.style.borderColor = 'var(--border)');\n"
        "  const card = document.getElementById('insight-' + cqKey);\n"
        "  if (card) card.style.borderColor = cqKey === 'cq2' ? 'var(--orange)' : 'var(--red)';\n"
        "};\n"
        "\n"
        "// Select node by ID (called from detail panel neighbor links)\n"
        "window._ontoSelectById = function(id) {\n"
        "  if (!window._ontoNodeMap || !window._ontoNodeMap[id]) return;\n"
        "  window._ontoSelectedNode = window._ontoNodeMap[id];\n"
        "  window._ontoHighlightSet = null;\n"
        "  showNodeDetail(window._ontoNodeMap[id]);\n"
        "  if (window._ontoDraw) window._ontoDraw();\n"
        "};\n"
        "\n"
        "function initOntoCanvas() {\n"
        "  if (_ontoInitialized) return;\n"
        "  const canvas = document.getElementById('onto-canvas');\n"
        "  if (!canvas || !_ONTO.nodes.length) return;\n"
        "  const rect = canvas.getBoundingClientRect();\n"
        "  if (rect.width < 10) return;\n"
        "  _ontoInitialized = true;\n"
        ...
        "  window._ontoNodes = nodes;\n"
        "  window._ontoNodeMap = nodeMap;\n"
        "  window._ontoEdges = edges;\n"
        "\n"
        ...
        "\n"
        "  // Get neighbor set for a node\n"
        "  function getNeighborIds(nodeId) {\n"
        "    const s = new Set();\n"
        "    edges.forEach(e => { if (e.s===nodeId) s.add(e.t); if (e.t===nodeId) s.add(e.s); });\n"
        "    return s;\n"
        "  }\n"
        "\n"
        "  function draw() {\n"
        ...
        "      let a = nodeMap[e.s], b = nodeMap[e.t];\n"
        "      if (!a || !b) return;\n"
        "      let isSelected = sel && (sel.id===e.s || sel.id===e.t);\n"
        "      let isHovered = hovered && (hovered.id===e.s || hovered.id===e.t);\n"
        "      let isHL = hlSet && (hlSet.has(e.s) || hlSet.has(e.t));\n"
        ...
        "      let isSel = sel && sel.id === n.id;\n"
        "      let isNeighbor = sel && selNeighbors && selNeighbors.has(n.id);\n"
        "      let isHL = hlSet && hlSet.has(n.id);\n"
        "      let dimmed = (sel || hlSet) && !isSel && !isNeighbor && !isHL && !isHov;\n"
        "\n"
        "      if (isSel) { r *= 2.0; ctx.shadowColor = color; ctx.shadowBlur = 20; }\n"
        "      else if (isNeighbor) { r *= 1.4; ctx.shadowColor = color; ctx.shadowBlur = 8; }\n"
        "      else if (isHL) { r *= 1.5; ctx.shadowColor = color; ctx.shadowBlur = 10; }\n"
        "      else if (isHov) { r *= 1.6; ctx.shadowColor = color; ctx.shadowBlur = 12; }\n"
        "\n"
        "      let alpha = dimmed ? '33' : '88';\n"
        "      let coreAlpha = dimmed ? color + '55' : color;\n"
        "      let grad = ctx.createRadialGradient(n.x-r*0.3, n.y-r*0.3, 0, n.x, n.y, r);\n"
        "      grad.addColorStop(0, coreAlpha); grad.addColorStop(1, color + alpha);\n"
        "      ctx.fillStyle = grad;\n"
        ...
        "      ctx.shadowBlur = 0;\n"
        "\n"
        "      // Labels for hovered/selected/neighbor/highlighted nodes\n"
        "      if (isHov || isSel || isNeighbor || isHL) {\n"
        "        ctx.fillStyle = dimmed ? 'rgba(255,255,255,0.3)' : '#fff';\n"
        "        ctx.font = (isSel || isHov) ? '11px -apple-system,sans-serif' : '9px -apple-system,sans-serif';\n"
        "        ctx.textAlign = 'center';\n"
        "        ctx.fillText(n.label || n.class, n.x, n.y - r - 6);\n"
        "        if (isSel || isHov) {\n"
        "          ctx.font = '9px monospace'; ctx.fillStyle = color;\n"
        "          ctx.fillText(n.class, n.x, n.y - r - 18);\n"
        "        }\n"
        "      }\n"
        "    });\n"
        "\n"
        ...
        "    const br = canvas.getBoundingClientRect();\n"
        "    mouseDownPos = {x: e.clientX - br.left, y: e.clientY - br.top};\n"
        "    if (hovered) { dragging = hovered; canvas.style.cursor = 'grabbing'; }\n"
        "  });\n"
        "  canvas.addEventListener('mouseup', e => {\n"
        "    const br = canvas.getBoundingClientRect();\n"
        "    const mx = e.clientX - br.left, my = e.clientY - br.top;\n"
        "    // Click detection: mousedown→mouseup distance < 5px\n"
        "    if (mouseDownPos && Math.abs(mx-mouseDownPos.x)<5 && Math.abs(my-mouseDownPos.y)<5) {\n"
        "      if (hovered) {\n"
        "        window._ontoSelectedNode = hovered;\n"
        "        window._ontoHighlightSet = null;\n"
        "        document.querySelectorAll('.onto-insight-card').forEach(c => c.style.borderColor = 'var(--border)');\n"
        "        showNodeDetail(hovered);\n"
        "      } else {\n"
        "        window._ontoSelectedNode = null;\n"
        "        window._ontoHighlightSet = null;\n"
        "        document.getElementById('onto-detail').style.display = 'none';\n"
        "        document.querySelectorAll('.onto-insight-card').forEach(c => c.style.borderColor = 'var(--border)');\n"
        "      }\n"
        "    }\n"
        "    dragging = null; mouseDownPos = null;\n"
        "    canvas.style.cursor = hovered ? 'pointer' : 'grab';\n"
        "    draw();\n"
        "  });\n"
        "  canvas.addEventListener('mouseleave', () => { hovered=null; dragging=null; mouseDownPos=null; draw(); });\n"
        "}\n"
        "document.querySelectorAll('.tab').forEach(t => {\n"
        "  t.addEventListener('click', () => { if(t.dataset.tab==='ontology') setTimeout(initOntoCanvas, 50); });\n"
        "});\n"
        "initOntoCanvas();\n"
        "\n"
        "</script>"
